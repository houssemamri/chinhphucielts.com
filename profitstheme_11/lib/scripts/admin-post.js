jQuery(document).ready(function() {
	if (jQuery("#lp-social-tw-txt").length){
		var maxlimit = 140;
		var length = jQuery("#lp-social-tw-txt").val().length;
	
		if(length >= maxlimit) {
		jQuery("#lp-social-tw-txt").val(jQuery("#lp-social-tw-txt").val().substring(0, maxlimit));
			length = maxlimit;
		}
		
		jQuery(".counter").html( (maxlimit - length) + " characters left");
			
		jQuery("#lp-social-tw-txt").keyup(function(){
	
			var new_length = jQuery("#lp-social-tw-txt").val().length;
			if(new_length >= maxlimit) {
					jQuery("#lp-social-tw-txt").val(jQuery("#lp-social-tw-txt").val().substring(0, maxlimit));
					new_length = maxlimit;
			}
		
			jQuery(".counter").html( (maxlimit - new_length) + " characters left");
		});
    }
	...
	var regbtntype = jQuery('#lp-register-btntype').val();

	if ( regbtntype == 'blank' ) {
		jQuery('#lp-register-btnpremade_select-field').hide();
		jQuery('#lp-register-btnclr_select-field').show();
		jQuery('#lp-register-btntxt_text-field').show();
		jQuery('#lp-register-btn-img_upload-field').hide();
	} else if ( regbtntype == 'upload' ) {
		jQuery('#lp-register-btnpremade_select-field').hide();
		jQuery('#lp-register-btnclr_select-field').hide();
		jQuery('#lp-register-btntxt_text-field').hide();
		jQuery('#lp-register-btn-img_upload-field').show();
	} else {
		jQuery('#lp-register-btnpremade_select-field').show();
		jQuery('#lp-register-btnclr_select-field').hide();
		jQuery('#lp-register-btntxt_text-field').hide();
		jQuery('#lp-register-btn-img_upload-field').hide();
	}

	jQuery("#lp-register-btntype").change(function() {
        	var regbtntype = jQuery("option:selected", this).val();
        	if ( regbtntype == 'premade' ) {
			jQuery('#lp-register-btnpremade_select-field').show();
			jQuery('#lp-register-btnclr_select-field').hide();
			jQuery('#lp-register-btntxt_text-field').hide();
			jQuery('#lp-register-btn-img_upload-field').hide();
		} else if ( regbtntype == 'blank' ) {
			jQuery('#lp-register-btnpremade_select-field').hide();
			jQuery('#lp-register-btnclr_select-field').show();
			jQuery('#lp-register-btntxt_text-field').show();
			jQuery('#lp-register-btn-img_upload-field').hide();
	...
	if ( !jQuery('#lp-page-exit').is(":checked") ) {
		jQuery('#lp-page-exit-wrap').hide();
	}

	jQuery('#lp-page-exit').click(function(){
		if ( this.checked == true ) {
			jQuery("#lp-page-exit-wrap").show();
		} else {
			jQuery("#lp-page-exit-wrap").hide();
		}				
	});
	...
	if ( !jQuery('#lp-custom-background').is(":checked") ) {
		jQuery('#lp-custom-background-wrap').hide();
	}

	jQuery('#lp-custom-background').click(function(){
		if ( this.checked == true ) {
			jQuery("#lp-custom-background-wrap").show();
		} else {
			jQuery("#lp-custom-background-wrap").hide();
		}				
	});

	if ( !jQuery('#lp-custom-content-background').is(":checked") ) {
		jQuery('#lp-custom-content-background-wrap').hide();
	}

	jQuery('#lp-custom-content-background').click(function(){
		if ( this.checked == true ) {
			jQuery("#lp-custom-content-background-wrap").show();
		} else {
			jQuery("#lp-custom-content-background-wrap").hide();
		}				
	});

	if ( !jQuery('#lp-custom-footer-background').is(":checked") ) {
		jQuery('#lp-custom-footer-background-wrap').hide();
	}

	jQuery('#lp-custom-footer-background').click(function(){
		if ( this.checked == true ) {
			jQuery("#lp-custom-footer-background-wrap").show();
		} else {
			jQuery("#lp-custom-footer-background-wrap").hide();
		}				
	});

	if ( jQuery('#lp-register-disable-name').is(":checked") ) {
		jQuery('#lp-register-name_text-field').hide();
	}

	jQuery('#lp-register-disable-name').click(function(){
		if ( this.checked == true ) {
			jQuery("#lp-register-name_text-field").hide();
		} else {
			jQuery("#lp-register-name_text-field").show();
		}				
	});
	...
	var vidplayer = jQuery('#lp-video-player').val();
	if ( vidplayer == 'html5' ) { 
		jQuery("#lp-video-html5").show();
		jQuery("#lp-video-control_checkbox-field").hide();
		jQuery("#lp-video-url_text-field").hide();
	} else {
		jQuery("#lp-video-html5").hide();
		jQuery("#lp-video-control_checkbox-field").show();
		jQuery("#lp-video-url_text-field").show();
	}

	jQuery('#lp-video-player').change(function(){
		var vidplayer = jQuery("option:selected", this).val();
		if ( vidplayer == 'html5' ) { 
			jQuery("#lp-video-html5").show();
			jQuery("#lp-video-control_checkbox-field").hide();
			jQuery("#lp-video-url_text-field").hide();
	...
	//2
	var vidplayer = jQuery('#lp-video-2-player').val();
	if ( vidplayer == 'html5' ) { 
		jQuery("#lp-video-2-html5").show();
		jQuery("#lp-video-2-control_checkbox-field").hide();
		jQuery("#lp-video-2-url_text-field").hide();
	} else {
		jQuery("#lp-video-2-html5").hide();
		jQuery("#lp-video-2-control_checkbox-field").show();
		jQuery("#lp-video-2-url_text-field").show();
	}

	jQuery('#lp-video-2-player').change(function(){
		var vidplayer = jQuery("option:selected", this).val();
		if ( vidplayer == 'html5' ) { 
			jQuery("#lp-video-2-html5").show();
			jQuery("#lp-video-2-control_checkbox-field").hide();
	...
	var mvidplayer = jQuery('#member-video-player').val();
	if ( mvidplayer == 'html5' ) { 
		jQuery("#member-video-html5").show();
		jQuery("#member-video-ctrl_checkbox-field").hide();
		jQuery("#member-video-url_text-field").hide();
	} else {
		jQuery("#member-video-html5").hide();
		jQuery("#member-video-ctrl_checkbox-field").show();
		jQuery("#member-video-url_text-field").show();
	}

	jQuery('#member-video-player').change(function(){
		var mvidplayer = jQuery("option:selected", this).val();
		if ( mvidplayer == 'html5' ) { 
			jQuery("#member-video-html5").show();
			jQuery("#member-video-ctrl_checkbox-field").hide();
			jQuery("#member-video-url_text-field").hide();
